# jevmory — team mission

## Outcome

Ship a viral-ready, local, zero-dependency developer tool: **jevmory** — coding-agent
...
3. Real `jevmory.md` file generated from real transcripts on this machine, containing graded verbatim facts with receipts. (Adjudicated with Kimi F9-nit: this real-transcript dream run is part of the smoke budget — capped at ~40 API requests, usage logged in `runs.stats`.)
4. `jevmory audit` produces a graded report on a real or realistic MEMORY.md.
...
`romiluz13/jevmory` immediately, with full OSS scaffolding. Still human-reserved:
jev.directory submission, tweeting, PyPI publishing.

## Roster

- **Lead** (w1S:p3): decisions, plan, verification, packaging. Writes docs/, decisions.
- **GLM** (w1S:p4): implementation. Owns all code under `jevmory/` + `tests/` + `scripts/`.
- **Kimi** (w1S:p5): devil's advocate + code review. Read-only on code; writes `.team/kimi/` notes.

## Working agreement

- Method: DDD (`.ddd/notes/jevmory.md` is the single task note; documentation basis must stay updated).
- User awake since 2026-09-19 morning; push to GitHub (public, romiluz13/jevmory) authorized. No publishing beyond GitHub (PyPI, jev.directory, social) without asking. API spend still limited to smoke tests.
...
- Name: `jevmory` (package/CLI), output file `jevmory.md`.
- MIT license. Version 0.1.0.
- Per-project SQLite store under `~/.jevmory/`.
