"""dream-md command line entry point: the M6 wiring surface.

    dream-md init [--enable-grading] [--project DIR]
    dream-md ingest --transcript PATH | --scan [--project DIR] [--test]
    dream-md dream   [--project DIR] [--offline] [--force]
    dream-md audit   [MEMORY.md] [--project DIR] [--json|--md] [--offline]
    dream-md resolve <fact_id> --keep-new|--keep-old [--project DIR]
    dream-md status  [--project DIR]
    dream-md install --agent claude|codex [--project DIR] [--yes]
...
(``python3 -m dream_md.hook``) is NOT this module: it never exits
...
from dream_md.audit.report import render_json, render_md, render_terminal
from dream_md.dream.engine import GradingNotEnabledError, run_dream
from dream_md.dream.resolve import KEEP_NEW, KEEP_OLD, resolve
...
from dream_md.ingestion.extract import candidates_from_statements
...
from dream_md.judgment.client import JevClient
from dream_md.judgment.errors import JevError
from dream_md.judgment.fake import MODE_NORMAL, FakeJev
from dream_md.memory.facts import ask_facts
from dream_md.memory.schema import connect, migrate

# Payload keys the hook recognizes (kept in sync with dream_md.hook).
...
_ENABLE_GRADING_HINT = (
    ...
    "  enable:  dream-md init --enable-grading\n"
    "  stay local (no API ever):  dream-md dream --offline"
)
_KEY_HINT = "$TYPESAFE_API_KEY is not set — grading needs it, or use --offline"


def _fail(message: str) -> int:
    print(f"dream-md: {message}", file=sys.stderr)
    return 1
...
def _offline_client() -> FakeJev:
    ...
    # real dream.md and a real report (the demo of shapes, not of doubt).
...
def _cmd_init(args, stdin_text: str | None) -> int:
    ...
    if args.enable_grading:
        ...
        print(
            "you have opted in: `dream-md dream` and `dream-md audit` may send\n"
    ...
    print(
        "ingest:   dream-md ingest --scan   (finds this project's transcripts\n"
        "          under ~/.claude/projects and ~/.codex/sessions — local only)\n"
        "hooks:    dream-md install --agent claude|codex   (prints config)"
    )
    return 0
...
def _ingest_scan(args) -> int:
    ...
    print("next: dream-md dream --offline   (local simulation, no API)")
    return 0
...
def _cmd_dream(args, stdin_text: str | None) -> int:
    ...
    artifact = Path(project) / "dream.md"
    try:
        write_dream_md(
            artifact,
            render_dream_md(
        ...
        return _fail(
            f"{artifact} exists without a dream-md sentinel; pass --force to "
            "overwrite it"
        )
    ...
    if report.asks:
        print(f"asks:         {list(report.asks)} — resolve with dream-md resolve <id>")
...
def _cmd_audit(args, stdin_text: str | None) -> int:
    ...
    if not memory_path.is_file():
        return _fail(
            ...
            "(positional argument, e.g. dream-md audit MEMORY.md)"
        )
...
def _cmd_resolve(args, stdin_text: str | None) -> int:
    ...
    print("next: dream-md dream   (dream.md regenerates with the resolution)")
    return 0
...
def _cmd_status(args, stdin_text: str | None) -> int:
    ...
    if not store.exists():
        ...
        print("first step:   dream-md ingest --scan")
        if not marker.exists():
            print("grading:      " + _ENABLE_GRADING_HINT.splitlines()[0])
            print("              dream-md init --enable-grading   dream-md dream --offline")
        return 0
    ...
    if artifact.exists():
        ...
        print(f"dream.md:     {artifact} ({'sentinel ok' if healthy else 'NO SENTINEL — not ours'})")
    else:
        print(f"dream.md:     not written yet (dream-md dream)")
            ...
            "SessionEnd": [
                {
                    "hooks": [
                        {
                            "type": "command",
                            "command": "python3 -m dream_md.hook",
                        }
                    ]
                }
            ]
...
def _codex_notify_line() -> str:
    return 'notify = ["python3", "-m", "dream_md.hook", "turn-ended"]'
...
def _install_claude(args) -> int:
    ...
    print(
        ...
        "transcript_path, cwd) to the hook on stdin; dream-md ingests that\n"
        "transcript into this project's local store and always exits 0."
    )
    if not args.yes:
        print("(print-only: dream-md never patches agent config without --yes)")
        return 0
...
def _patch_claude_settings() -> int:
    ...
    try:
        ...
        if any(
            "dream_md.hook" in json.dumps(entry) for entry in session_end
        ):
...
def _install_codex(args) -> int:
    ...
    print(
        ...
        "argv after each turn; dream-md scans argv (and stdin) for the payload,\n"
        "ingests the transcript it names into this project's local store, and\n"
        "always exits 0. If a payload shape ever changes, `dream-md ingest\n"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("dream", help="grade + consolidate; writes dream.md")
    p.add_argument("--project", metavar="DIR")
    p.add_argument(
        "--offline",
        action="store_true",
    ...
    p.add_argument(
        "--force", action="store_true", help="overwrite a foreign dream.md"
    )
