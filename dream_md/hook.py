...
`python3 -m dream_md.hook` reads one JSON payload from stdin (Claude
...
~/.dream-md/hooks/ so they are visible via `dream-md status` without
...
def log_path(project_dir: str, *, home: Path | None = None) -> Path:
    """Per-project hook log: <home>/.dream-md/hooks/<slug>.jsonl."""

    base = home if home is not None else default_home()
    return base / ".dream-md" / "hooks" / f"{project_slug(project_dir)}.jsonl"
...
def global_log_path(home: Path | None = None) -> Path:
    ...
    base = home if home is not None else default_home()
    return base / ".dream-md" / "hooks" / "global.jsonl"
