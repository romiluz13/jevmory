...
Roots (home resolves via DREAM_MD_HOME when set, for tests/sandboxes):
...
_HEAD_BYTES = 64 * 1024  # enough to cover session_meta + first turns
...
def discover_transcripts(
    ...
    session metadata (claude "cwd" / codex "payload.cwd"). Containment
...
def detect_source(path: str | os.PathLike[str]) -> str:
    """Guess whether a transcript came from claude or codex.
    ...
    whose payload has an "id"; claude sessions open with records carrying
    "sessionId". Raises ValueError when neither heuristic applies.
    """
    ...
    if '"sessionId"' in first or '"type":"mode"' in first.replace(" ", ""):
        return "claude"
    raise ValueError(f"cannot tell claude vs codex for {path!r}; use --agent")
