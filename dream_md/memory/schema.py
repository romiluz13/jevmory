...
the store lives here (review R7); ``dream_md.ingestion.eventlog`` writes
...
dream.md writes always supply text, so the difference is tolerated and
...
def migrate(conn: sqlite3.Connection) -> int:
    ...
    if version is None:
        ...
        raise SchemaVersionError(
            ...
            f"{SCHEMA_VERSION}; upgrade dream-md"
        )
    elif version < SCHEMA_VERSION:
