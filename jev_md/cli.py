"""jev-md command line entry point: the M6 wiring surface.

    jev-md init [--enable-grading] [--project DIR]
    jev-md ingest --transcript PATH | --scan [--project DIR] [--test]
    jev-md dream   [--project DIR] [--offline] [--force]
    jev-md audit   [MEMORY.md] [--project DIR] [--json|--md] [--offline]
    jev-md resolve <fact_id> --keep-new|--keep-old [--project DIR]
    jev-md status  [--project DIR]
    jev-md install --agent claude|codex [--project DIR] [--yes]
...
(``python3 -m jev_md.hook``) is NOT this module: it never exits
...
from jev_md.audit.report import render_json, render_md, render_terminal
from jev_md.dream.engine import GradingNotEnabledError, run_dream
from jev_md.dream.resolve import KEEP_NEW, KEEP_OLD, resolve
...
from jev_md.ingestion.extract import candidates_from_statements
from jev_md.ingestion.redact import redactions_in
from jev_md.ingestion.scan import detect_source, discover_transcripts
from jev_md.judgment.client import JevClient
from jev_md.judgment.errors import JevError
from jev_md.judgment.fake import MODE_NORMAL, FakeJev
from jev_md.memory.facts import ask_facts
from jev_md.memory.schema import connect, migrate

# Payload keys the hook recognizes (kept in sync with jev_md.hook).
...
_ENABLE_GRADING_HINT = (
    ...
    "  enable:  jev-md init --enable-grading\n"
    "  stay local (no API ever):  jev-md dream --offline"
)
_KEY_HINT = "$TYPESAFE_API_KEY is not set — grading needs it, or use --offline"


def _fail(message: str) -> int:
    print(f"jev-md: {message}", file=sys.stderr)
    return 1
...
def _offline_client() -> FakeJev:
    ...
    # real jev.md and a real report (the demo of shapes, not of doubt).
...
def _cmd_init(args, stdin_text: str | None) -> int:
    ...
    if args.enable_grading:
        ...
        print(
            "you have opted in: `jev-md dream` and `jev-md audit` may send\n"
    ...
    print(
        "ingest:   jev-md ingest --scan   (finds this project's transcripts\n"
        "          under ~/.claude/projects and ~/.codex/sessions — local only)\n"
        "hooks:    jev-md install --agent claude|codex   (prints config)"
    )
    return 0
...
def _ingest_scan(args) -> int:
    ...
    print("next: jev-md dream --offline   (local simulation, no API)")
    return 0
...
def _cmd_dream(args, stdin_text: str | None) -> int:
    ...
    try:
        ...
        return _fail(
            f"{artifact} exists without a jev-md sentinel; pass --force to "
            "overwrite it"
        )
    ...
    if report.asks:
        print(f"asks:         {list(report.asks)} — resolve with jev-md resolve <id>")
...
def _cmd_audit(args, stdin_text: str | None) -> int:
    ...
    if not memory_path.is_file():
        return _fail(
            ...
            "(positional argument, e.g. jev-md audit MEMORY.md)"
        )
...
def _cmd_resolve(args, stdin_text: str | None) -> int:
    ...
    print("next: jev-md dream   (jev.md regenerates with the resolution)")
    return 0
...
def _cmd_status(args, stdin_text: str | None) -> int:
    ...
    if not store.exists():
        ...
        print("first step:   jev-md ingest --scan")
        if not marker.exists():
            print("grading:      " + _ENABLE_GRADING_HINT.splitlines()[0])
            print("              jev-md init --enable-grading   jev-md dream --offline")
        return 0
    ...
    if artifact.exists():
        ...
        print(f"jev.md:     {artifact} ({'sentinel ok' if healthy else 'NO SENTINEL — not ours'})")
    else:
        print(f"jev.md:     not written yet (jev-md dream)")
...
def _codex_notify_line() -> str:
    return 'notify = ["python3", "-m", "jev_md.hook", "turn-ended"]'
...
def _install_claude(args) -> int:
    ...
    print(
        ...
        "transcript_path, cwd) to the hook on stdin; jev-md ingests that\n"
        "transcript into this project's local store and always exits 0."
    )
    if not args.yes:
        print("(print-only: jev-md never patches agent config without --yes)")
        return 0
...
def _patch_claude_settings() -> int:
    ...
    try:
        ...
        if any(
            "jev_md.hook" in json.dumps(entry) for entry in session_end
        ):
...
def _install_codex(args) -> int:
    ...
    print(
        ...
        "argv after each turn; jev-md scans argv (and stdin) for the payload,\n"
        "ingests the transcript it names into this project's local store, and\n"
        "always exits 0. If a payload shape ever changes, `jev-md ingest\n"
...
def _cmd_hook(args, stdin_text: str | None) -> int:
    ...
    through ``python3 -m jev_md.hook`` (T3: the alias used to pass
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--version", action="version", version=f"jev-md {__version__}"
    )
    ...
    p = sub.add_parser("dream", help="grade + consolidate; writes jev.md")
    p.add_argument("--project", metavar="DIR")
    p.add_argument(
        "--offline",
        action="store_true",
    ...
    p.add_argument(
        "--force", action="store_true", help="overwrite a foreign jev.md"
    )
