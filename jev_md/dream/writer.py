"""jev.md writer (PLAN M5): render the memory artifact, guard its sentinel.
...
Layout (PLAN "jev.md output format"):
...
- the sentinel comment line first — the guard's version-independent
  ...
  older jev-md version is still recognized as ours;
...
``write_jev_md`` is the sentinel guard: a path that exists without
...
from jev_md.thresholds import STALENESS_BADGE_DAYS
...
SENTINEL = (
    f"<!-- jev-md v{__version__} sentinel — generated file, do not edit; "
    "regenerate with `jev-md dream` -->"
)
# Version-independent core: recognizes files written by ANY jev-md version.
...
class SentinelError(Exception):
    """Refusing to overwrite a file that is not jev-md's own."""
...
def render_jev_md(
    ...
    *,
    ...
    """Render the jev.md artifact. Pure; see module docstring.
    ...
    lines: list[str] = [SENTINEL, "# jev.md", ""]
...
def write_jev_md(
    ...
    """Write the artifact, refusing to clobber a non-jev-md file.
    ...
    if target.exists():
        ...
        if SENTINEL_CORE not in existing and not force:
            raise SentinelError(
                f"{target} exists and has no jev-md sentinel; pass --force "
                "to overwrite it"
            )
...
def _ask_line(ask: AskPair) -> str:
    ...
    line += (
        f" `jev-md resolve {ask.fact.id} --keep-new|--keep-old`"
    )
    return line
