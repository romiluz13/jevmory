...
`python3 -m jev_md.hook` reads one JSON payload from stdin (Claude
...
~/.jev-md/hooks/ so they are visible via `jev-md status` without
...
def log_path(project_dir: str, *, home: Path | None = None) -> Path:
    """Per-project hook log: <home>/.jev-md/hooks/<slug>.jsonl."""

    base = home if home is not None else default_home()
    return base / ".jev-md" / "hooks" / f"{project_slug(project_dir)}.jsonl"
...
def global_log_path(home: Path | None = None) -> Path:
    ...
    base = home if home is not None else default_home()
    return base / ".jev-md" / "hooks" / "global.jsonl"
