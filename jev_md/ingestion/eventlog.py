"""SQLite Event log: the events table (DDL owned by ``jev_md.memory.schema``).
...
from jev_md.ingestion.models import ParsedTranscript
from jev_md.ingestion.redact import redact
from jev_md.memory.schema import connect, migrate

# No DDL here (review R7): every CREATE lives in jev_md.memory.schema.
...
def default_home() -> Path:
    """Root for jev-md state. ``$JEV_MD_HOME`` wins over ``Path.home()``.
    ...
    ``~/.codex/config.toml``) are NOT jev-md state and always use the
    real home.
    """

    override = os.environ.get("JEV_MD_HOME")
...
def store_path(
    ...
    """SQLite store path for a project (PLAN: ~/.jev-md/projects/<slug>.db)."""
    base = Path(home) if home is not None else default_home()
    return base / ".jev-md" / "projects" / f"{project_slug(project_dir)}.db"


def optin_path(
    project_dir: str | os.PathLike[str], home: str | os.PathLike[str] | None = None
) -> Path:
    """Grading opt-in marker path (~/.jev-md/projects/<slug>.optin).
    ...
    created by ``jev-md init --enable-grading``. No marker -> the
    dream engine refuses to grade and candidates stay queued.
    """
    base = Path(home) if home is not None else default_home()
    return base / ".jev-md" / "projects" / f"{project_slug(project_dir)}.optin"
