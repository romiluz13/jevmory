...
the store lives here (review R7); ``jev_md.ingestion.eventlog`` writes
...
jev.md writes always supply text, so the difference is tolerated and
...
def migrate(conn: sqlite3.Connection) -> int:
    ...
    if version is None:
        ...
        raise SchemaVersionError(
            ...
            f"{SCHEMA_VERSION}; upgrade jev-md"
        )
    elif version < SCHEMA_VERSION:
