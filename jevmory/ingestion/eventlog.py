...
from jevmory.ingestion.models import ParsedTranscript
from jevmory.ingestion.redact import redact
from jevmory.memory.schema import connect, migrate

# No DDL here (review R7): every CREATE lives in jevmory.memory.schema.
...
def default_home() -> Path:
    """Root for jevmory state. ``$JEVMORY_HOME`` wins over ``Path.home()``.
    ...
    ``~/.codex/config.toml``) are NOT jevmory state and always use the
    real home.
    """

    override = os.environ.get("JEVMORY_HOME")
...
def store_path(
    ...
    """SQLite store path for a project (PLAN: ~/.jevmory/projects/<slug>.db)."""
    base = Path(home) if home is not None else default_home()
    return base / ".jevmory" / "projects" / f"{project_slug(project_dir)}.db"


def optin_path(
    project_dir: str | os.PathLike[str], home: str | os.PathLike[str] | None = None
) -> Path:
    """Grading opt-in marker path (~/.jevmory/projects/<slug>.optin).
