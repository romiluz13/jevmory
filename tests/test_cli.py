...
`main(argv, stdin_text)` is driven in-process with JEVMORY_HOME
...
The one part of the CLI that writes OUTSIDE jevmory state —
...
class CliTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Redirect every jevmory state path into the tmp home, and keep
        # the live API gate deterministically closed unless a test opens it.
        self._env = {
            name: os.environ.get(name)
            for name in ("JEVMORY_HOME", "TYPESAFE_API_KEY")
        }
    ...
    def test_init_prints_paths_and_writes_nothing(self):
        ...
        self.assertIn("jevmory init --enable-grading", out)
    ...
    def test_init_enable_grading_creates_the_marker(self):
        ...
        marker = self.home / ".jevmory" / "projects" / f"{self.slug}.optin"
    ...
    def test_status_before_any_store(self):
        ...
        self.assertIn("first step:   jevmory ingest --scan", out)
    ...
    def test_audit_offline_prints_the_terminal_report(self):
        ...
        self.assertEqual(rc, 0)
        self.assertIn("jevmory audit — ", out)
...
class InstallPatchTest(unittest.TestCase):
    ...
    def patched(self, name: str, target: Path):
        return mock.patch(f"jevmory.cli.{name}", return_value=target)
    ...
    def test_patch_claude_settings_from_scratch(self):
        ...
        self.assertEqual(command, "python3 -m jevmory.hook")
    ...
    def test_patch_codex_config_from_scratch(self):
        ...
        self.assertIn('notify = ["python3", "-m", "jevmory.hook", "turn-ended"]', text)
    ...
    def test_patch_codex_config_replaces_an_existing_notify(self):
        ...
        self.assertEqual(rc, 0)
        text = target.read_text(encoding="utf-8")
        self.assertIn("jevmory.hook", text)
