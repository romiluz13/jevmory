...
never touching the real ``~/.jevmory``. These tests pin that promise;
...
def _phase_c_triple(i):
    from jevmory.audit.questions import phase_c_triple
...
class DemoCommandTest(unittest.TestCase):
    ...
    def test_main_runs_clean_via_subprocess(self):
        ...
        self.assertIn("jevmory init --enable-grading", result.stdout)
