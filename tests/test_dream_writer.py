"""dream.md writer tests (M5): pure rendering (pinned order, sorting,
...
class RenderDreamMdTest(unittest.TestCase):
    ...
    def test_categories_in_pinned_order_without_none(self):
        ...
        md = render_dream_md(facts, now=NOW)
    ...
    def test_asks_render_questions_section(self):
        ...
        md = render_dream_md([], asks, now=NOW)
        ...
        self.assertIn("`dream-md resolve 7 --keep-new|--keep-old`", md)
        # a missing partner link still renders with the resolve command
        self.assertIn("`dream-md resolve 8 --keep-new|--keep-old`", md)
    ...
    def test_render_is_pure_byte_identical(self):
        ...
        first = render_dream_md(facts, asks, now=NOW)
        second = render_dream_md(facts, asks, now=NOW)
...
class WriteDreamMdTest(unittest.TestCase):
    def setUp(self):
        ...
        self.path = Path(self._tmp.name) / "dream.md"

    def tearDown(self):
        self._tmp.cleanup()

    def test_fresh_path_writes(self):
        write_dream_md(self.path, render_dream_md([], now=NOW))
    ...
    def test_older_version_sentinel_still_regenerates(self):
        old = "<!-- dream-md v0.0.1 sentinel — generated file, do not edit; " \
    ...
    def test_force_overrides_foreign_file(self):
        self.path.write_text("someone's hand-written notes\n")
        write_dream_md(self.path, render_dream_md([], now=NOW), force=True)
        self.assertIn("# dream.md", self.path.read_text())
